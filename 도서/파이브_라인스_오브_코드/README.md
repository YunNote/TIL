...
### 함수 분해를 위한 리팩터링 패턴 소개.
 ...
 - 처음에는 함수명을 고려한다. 각 줄을 모두 이해하려다 보면 시간이 많이 걸리고 비생산적인 수령에 빠질 수 있다.

함수 전체를 소화하려고 하지 말고, 작게 잘라서 이해하기 쉽게 하나씩 처리해야한다.
...
> 
> 여러 매개변수가 필요하거나 여러 주건에서 전체가 아닌 일부조건에섬나 return할 경우 복잡해 질 수 있다.
> 
> tip - if의 일부분기만 return문을 가지고 있는 경우 메서드를 추출하는데 방해가 될 수 있따. 따라서 메서드의 끝에서 시작해 위로 작업해가는것이 좋다.
> 이는 return 문을 가진 조건을 메서드의 앞쪽에 배치하게 해서 결과적으로 모든 분기에서 return 할 수 있다.
